...
importEnviroCar = function(file) {
  ...
  layer$time = as.POSIXct(as.character(layer$time), format="%Y/%m/%d %H:%M:%S", tz="UTC")
...
The GPS trajectory must be a {\tt SpatialPointsDataFrame} as defined in {\tt sp}
or one of the {\tt Track} classes defined in {\tt trajectories}.
...
@
The result is an object of the same class as the input containing the the new map matched 
